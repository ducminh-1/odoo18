...
class WooInstanceEpt(models.Model):
    ...
    # Analytic
    woo_is_use_analytic_account = fields.Boolean(default=True)
    ...
    is_woo_digest = fields.Boolean(string="Set Woo Digest?")
    ...
    woo_ship_order_webhook = fields.Boolean("Want to ship order while importing order",
                                            help="If checked, it will fulfill order in odoo")
    woo_forcefully_reserve_stock_webhook = fields.Boolean("ForceFully Reserve Stock while importing order",
                                                          help="If checked, It will forcefully reserve stock in the picking")
    woo_refund_order_webhook = fields.Boolean("Want to create a refund while importing order",
                                              help="If checked, it will create a refund in odoo")

    woo_customer_order_webhook = fields.Boolean("Want to update customer while importing order",
                                                help="If checked, it will update the customer in order")
    woo_update_qty_order_webhook = fields.Boolean("Want to update Quantity while importing order",
                                                  help="If checked, it will update the customer in order")
    woo_add_new_product_order_webhook = fields.Boolean("Want to Add New Product while importing order",
                                                       help="If checked, it will add new product in the order if receive in the webhook")
    woo_remove_product_order_webhook = fields.Boolean("Want to Remove Product while importing order",
    ...
    woo_credit_note_register_payment = fields.Boolean("Want to create register payment for credit note",
                                                      help="If checked, it will create a payment for credit note")
    woo_credit_note_payment_journal = fields.Many2one("account.journal", string="Credit Note Payment Journal",
    ...
    def get_compare_data(self, record):
        ...
        def get_compared_week_data(record):
            ...
            qry = """select sum(amount_untaxed) as current_week from sale_order
                                    ...
                                    woo_instance_id=%s and state in ('sale','done')"""
            self._cr.execute(qry, (record.id,))
    ...
    def get_total_orders(self):
        ...
        """
        order_query = """select id from sale_order where woo_instance_id= %s and state in ('sale','done')"""
        ...
        def orders_of_all_time(record):
            qry = """select id from sale_order where woo_instance_id = %s and state in ('sale','done')"""
            self._cr.execute(qry, (record.id,))
            return self._cr.dictfetchall()
    ...
    def get_total_products(self):
        ...
        """
        product_data = {}
        qry = """select count(id) as total_count from woo_product_template_ept where
                            exported_in_woo = True and woo_instance_id = %s"""
        self._cr.execute(qry, (self.id,))
    ...
    def get_customers(self):
        ...
        """
        customer_data = {}
        qry = """select partner_id from woo_res_partner_ept where woo_instance_id = %s"""
        self._cr.execute(qry, (self.id,))
        result = self._cr.dictfetchall()
    ...
    def get_refund(self):
        ...
        """
        refund_query = """select id from account_move where woo_instance_id=%s and move_type='out_refund'"""
        ...
        try:
            ...
            if hasattr(error, "response"):
                if error.response.code == 403:
                    raise UserError(
                        _("Need to Enable XMLRPC.PHP to woo commerce store,\n\n Please refer to link for more details (https://bit.ly/owc-xmlrpc-tn-faq)"))
                else:
