...
class WooProductTemplateEpt(models.Model):
    ...
    woo_description = fields.Html("Description", translate=True, sanitize=False)
    woo_short_description = fields.Html("Short Description", translate=True, sanitize=False)
    ...
    def get_product_price(self, instance, variant):
        ...
        """
        vals = {}
        price, regular_price_rule = instance.woo_pricelist_id._get_product_price_rule(variant.product_id,
                                                                                      quantity=1.0,
                                                                                      uom_id=variant.product_id.uom_id.id)
        vals.update({'regular_price': str(price)})
        if instance.woo_extra_pricelist_id:
            sale_price, sale_price_rule = instance.woo_extra_pricelist_id._get_product_price_rule(
                variant.product_id,
                quantity=1.0, uom_id=variant.product_id.uom_id.id)
            if sale_price_rule:
    ...
    def woo_create_queue_for_export_stock(self, instance, woo_templates):
        ...
        if not warehouse:
            message = "The warehouse is not set for stock export. Please check the configuration and ensure that the appropriate warehouse is assigned for stock exports."
            common_log_line_obj.create_common_log_line_ept(operation_type='export',
                                                           module="woocommerce_ept",
                                                           woo_instance_id=instance.id,
                                                           model_name=self._context.get('active_model'),
                                                           res_id='', message=message,
    ...
    def prepare_variable_batch_export_stock_data(self, woo_template, product_stock):
        ...
        """
        variations = []
        woo_product_product_ids = woo_template.woo_product_ids.filtered(lambda x:
                                                                        x.product_id.type == 'consu' and x.product_id.is_storable == True
                                                                        and x.variant_id and x.woo_is_manage_stock)
        for variant in woo_product_product_ids:
    ...
    def woo_import_all_attributes(self, wc_api, response, instance):
        ...
        if int(total_pages) >= 2:
            for page in range(2, int(total_pages) + 1):
                ...
                data = self.check_woocommerce_response(res, "WooCommerce Attributes", "woo.product.attribute.ept",
                                                       instance)
    ...
    def sync_woo_attribute(self, instance):
        ...
        if isinstance(attributes_data, list):
            ...
            specific_product_attribute_ids = self._context.get('specific_product_attribute_ids')
            if specific_product_attribute_ids:
                attributes_data = list(
                    filter(lambda attributes: attributes.get('id') in specific_product_attribute_ids, attributes_data))
                self.find_or_create_woo_attribute(attributes_data, instance)
                self.sync_woo_attribute_term(instance,specific_product_attribute_ids)
                return True
    ...
    def create_woo_product_queue(self, results, instance, import_all, template_id):
        ...
        if total_result and not template_id:
            ...
            for queue in queues:
                if not queue.queue_line_ids:
                    queue.unlink()
                else:
                    product_queue_ids = queue.mapped('id')
                    _logger.info("Product Queue %s created", queues.mapped('name'))
                    ...
                    #                       "warning": True})
            self._cr.commit()
            process_product_cron = self.env.ref('woo_commerce_ept.process_woo_product_data',
                                                raise_if_not_found=False)
            if not process_product_cron:
                raise UserError(
    ...
    def search_odoo_product_variant(self, woo_instance, product_sku, variant_id):
        ...
        if product_sku and not woo_product:
    ...
    def woo_create_variant_product(self, product_template_dict, woo_instance):
        ...
        if attrib_line_vals:
            product_template_values = {'name': template_title,
                                       'type': 'consu',
                                       'is_storable':True,
        ...
        for product_data_queue_line in product_data_queue_lines:
            ...
            if data.get('type') not in ['simple', 'variable', 'bundle', 'grouped', 'external']:
                ...
                common_log_line_obj.create_common_log_line_ept(operation_type="import", module="woocommerce_ept",
                                                               ...
                                                               woo_order_data_queue_line_id=order_queue_line and order_queue_line.id)
    ...
    def template_attribute_process(self, woo_instance, odoo_template, variant, template_title, data,
        ...
        template_vals = {"name": template_title, "type": "consu",
                         "is_storable":True, "default_code": variant["sku"]}
    ...
    def get_product_attribute(self, template, instance):
        ...
        for attribute_line in template.attribute_line_ids:
            options = attribute_line.value_ids.with_context(lang=instance.woo_lang_id.code).mapped('name')
            variation = False
    ...
    def auto_update_product(self, ctx):
        """
        This method is call when auto update product cron in enable
        This method is call update_products() method which is responsible to update product in WooCommerce.
        :return: Boolean
        @author: Nilam Kubavat @Emipro Technologies Pvt. Ltd on date 09-10-2023.
        """
        woo_instance_id = ctx.get('woo_instance_id', False)
        instance = self.woo_instance_id.browse(woo_instance_id)
        process_import_export_obj = self.env["woo.process.import.export"]
        if instance:
            process_import_export_obj.update_products()
        return True

    def archive_deleted_product(self,woo_tmpl_ids):
        """
            This Method is used to archive product in woo layer which are deleted or have invalid id in woo store.
            @param woo_tmpl_ids: List of woo store product template Ids.
            @author: Gopal Chouhan @Emipro Technologies Pvt. Ltd on date 02-07-2024.
        """
        self.search([('woo_tmpl_id','in',woo_tmpl_ids)]).write({'active':False})
...
class ProductProductEpt(models.Model):
    ...
    woo_template_id = fields.Many2one("woo.product.template.ept", required=True, ondelete="cascade")
    active = fields.Boolean(default=True)
