...
class WooExportStockQueueEpt(models.Model):
    ...
    def create_woo_export_stock_queue(self, woo_instance, stock_data):
        ...
        while stock_data:
            ...
            if data:
                ...
                #                       "warning": True})
                self._cr.commit()
                process_export_stock_cron = self.env.ref('woo_commerce_ept.process_woo_export_stock_queue',
                                                         raise_if_not_found=False)
                if not process_export_stock_cron:
                    raise UserError(
    ...
    def create_queue_lines(self, stock_data):
        ...
        for data in stock_data:
            ...
            for batch_detail in batch_details:
                if type(batch_detail) == list:
                    for variant_data in batch_detail:
                        self.update_val_list(variant_data, vals_list, data)
                else:
                    self.update_val_list(batch_detail, vals_list, data)
                    # vals_list.append({
                    #     "batch_details": data.get("batch_details"),
    ...
    def update_val_list(self, variant_data, vals_list, final_data):
        product_id = self.env['woo.product.product.ept'].search([('variant_id', '=', variant_data.get('id'))])
        vals_list.append({
            "batch_details": final_data.get("batch_details"),
            "woo_tmpl_id": final_data.get("woo_tmpl_id"),
            "product_type": final_data.get("product_type"),
            "export_stock_queue_id": self.id,
            "quantity": variant_data.get('stock_quantity'),
            "woocommerce_product_id": product_id.id,
